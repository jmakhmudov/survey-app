...
CHOICES_HELP_TEXT = (
    """Для разделения вариантов ответа используйте символ |"""
)
...
class Question(models.Model):
    ...
    QUESTION_TYPES = (
        ...
        (SELECT_MULTIPLE, ("Закрытый вопрос")),
    )

    text = models.TextField(("Текст"))
    order = models.IntegerField(("Порядок"))
    required = models.BooleanField(("Обязателен"))
    survey = models.ForeignKey(Survey, on_delete=models.CASCADE, verbose_name=("Опросник"), related_name="questions")
    type = models.CharField(("Тип"), max_length=200, choices=QUESTION_TYPES, default=TEXT)
    choices = models.TextField(("Варианты"), blank=True, null=True, help_text=CHOICES_HELP_TEXT)
    ...
    def save(self, *args, **kwargs):
        if self.type in [Question.SELECT_MULTIPLE]:
            ...
            for value in answer.values:
                ...
                if value not in filter + standardized_filter:
                    if answer.response.user is None:
                        self._cardinality_plus_answer(cardinality, (settings.USER_DID_NOT_ANSWER), value)
    ...
    def _cardinality_plus_answer(self, cardinality, value, other_question_value):
        ...
        if cardinality.get(value) is None:
            ...
            cardinality[value] = {(settings.USER_DID_NOT_ANSWER): cardinality[value], other_question_value: 1}
        else:
